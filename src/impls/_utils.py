def _min_m_such_that_2n_minus_1_pow_k_ge_2p(
    p: int,
    n: int,
    hi: int,
) -> int:
...
def min_semisymbols_for_payload(p: int, n: int, hi: int) -> int:
    """
    Input:
        p: number of payload bits (p >= 0)
        n: 2**n - 1 to consider the alphabet size (n >= 1)
        hi: upper bound on the result
    Returns:
        m: minimal integer m >= 0 such that (2**n - 1)**m >= 2**p
    """
    return _min_m_such_that_2n_minus_1_pow_k_ge_2p(p, n, hi)
