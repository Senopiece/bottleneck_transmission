...
def monomials_gf(
    ...
    for i in range(m):
        out[i] = acc
        acc = gf_mul(acc, xx, n, mask, red)  # type: ignore

    return out


@nb.njit(inline="always")
def gf_pow(a: np.uint16, e: int, n: int, mask: np.uint16, red: np.uint16) -> np.uint16:
    res = np.uint16(1)
    base = a & mask
    ee = e
    while ee > 0:
        if ee & 1:
            res = gf_mul(res, base, n, mask, red)
        ee >>= 1
        if ee:
            base = gf_mul(base, base, n, mask, red)
    return res


@nb.njit(inline="always")
def gf_inv(a: np.uint16, n: int, mask: np.uint16, red: np.uint16) -> np.uint16:
    # In GF(2^n), a^{-1} = a^(2^n - 2) for a != 0
    if a == 0:
        # Numba can't raise Python exceptions cleanly in all contexts; return 0 and let caller detect.
        return np.uint16(0)
    return gf_pow(a, (1 << n) - 2, n, mask, red)


@nb.njit(cache=True)
def gf_solve_and_invert(
    ...
    for col in range(m):
        ...
        # Normalize pivot row
        inv_p = gf_inv(A[col, col], n, mask, red)
        if inv_p == 0:
            singular = True
            break

        for j in range(m):
            A[col, j] = gf_mul(A[col, j], inv_p, n, mask, red)
            InvT[col, j] = gf_mul(InvT[col, j], inv_p, n, mask, red)
        rhs[col] = gf_mul(rhs[col], inv_p, n, mask, red)
        ...
        for r in range(m):
            ...
            for j in range(m):
                A[r, j] ^= gf_mul(factor, A[col, j], n, mask, red)
                InvT[r, j] ^= gf_mul(factor, InvT[col, j], n, mask, red)
            rhs[r] ^= gf_mul(factor, rhs[col], n, mask, red)
...
def gf_matvec_right(
    ...
    for j in range(m):
        acc = np.uint16(0)
        for i in range(m):
            acc ^= gf_mul(v[i], X[i, j], n, mask, red)
        y[j] = acc
...
def step(
    ...
    return gf_dot(coeffs, monomials_gf(x, m, n, mask, red), n, mask, red)
