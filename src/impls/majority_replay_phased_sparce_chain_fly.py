...
# - replay peeling from majority observations before returning. This gives
#   repeated observations of the same x a chance to override an early corrupted y
#   while preserving linear-ish peeling complexity.
    ...
    def make_estimator() -> Estimator:
        ...
        def maybe_done() -> np.ndarray | None:
            ...
            def replay_domain(
                ...
                replay_symbols: List[int | None] = [None for _ in range(k)]
                replay_pending: List[Tuple[Set[int], int]] = []
                ordered_x = sorted(
                    observations,
                    key=lambda x: -max(observations[x].values()),
                )

                for x in ordered_x:
                    y = max(observations[x].items(), key=lambda item: item[1])[0]
                    peel_add_equation(
                        x,
                        y,
                        k,
                    ...
                    peel_propagate(replay_symbols, replay_pending)

                if any(v is None for v in replay_symbols):
                    return fallback
                return [int(v) for v in replay_symbols if v is not None]

            replayed_a = replay_domain(k1, cdf_a, salt_a, observed_a, repaired_a)
            replayed_b = replay_domain(k2, cdf_b, salt_b, observed_b, repaired_b)
