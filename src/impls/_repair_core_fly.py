...
EQUATION_BANK_REPAIR = False
REPAIR_EXTRA_PACKETS = 0
REPAIR_PASSES = 3
...
def create_protocol(config: Config) -> Protocol:
    ...
    def make_estimator() -> Estimator:
        ...
        equations_a: List[Tuple[List[int], int]] = []
        equations_b: List[Tuple[List[int], int]] = []
        singleton_votes_a: List[Dict[int, int]] = [dict() for _ in range(k1)]
        singleton_votes_b: List[Dict[int, int]] = [dict() for _ in range(k2)]
        ...
        repair_extra_left: int | None = None
        ...
        def residual_count(
            symbols: List[int | None], equations: List[Tuple[List[int], int]]
        ) -> int:
            conflicts = 0
            for subset, y in equations:
                ...
                for idx in subset:
                    ...
                    if value is None:
                        ...
                        break
                    lhs ^= int(value)
                if valid and lhs != int(y):
                    conflicts += 1
            return conflicts

        def repair_symbol_vector(
            symbols: List[int | None], equations: List[Tuple[List[int], int]]
        ) -> None:
            if not equations or any(value is None for value in symbols):
                return

            def equation_conflicts(candidate_symbols: List[int | None]) -> int:
                conflicts = 0
                for subset, y in equations:
                    lhs = 0
                    for idx in subset:
                        value = candidate_symbols[idx]
                        if value is None:
                            break
                        lhs ^= int(value)
                    else:
            ...
            def solve_equation_basis(order: np.ndarray, masks: List[int]) -> List[int] | None:
                k = len(symbols)
                basis_masks = [0 for _ in range(k)]
                basis_rhs = [0 for _ in range(k)]
                rank = 0

                for eq_idx_raw in order:
                    eq_idx = int(eq_idx_raw)
                    mask = int(masks[eq_idx])
                    rhs = int(equations[eq_idx][1]) & symbol_mask
                    while mask:
                        bit = mask.bit_length() - 1
                        if basis_masks[bit]:
                            mask ^= basis_masks[bit]
                            rhs ^= basis_rhs[bit]
                        else:
                            basis_masks[bit] = mask
                            basis_rhs[bit] = rhs
                            rank += 1
                            break
                    if rank == k:
                ...
                solved = [0 for _ in range(k)]
                for bit in range(k):
                    rhs = basis_rhs[bit]
                    mask = basis_masks[bit] & ~(1 << bit)
                    while mask:
                        low = mask & -mask
                        dep = low.bit_length() - 1
                        rhs ^= solved[dep]
                        mask ^= low
                    solved[bit] = rhs & symbol_mask

                return solved

            if (
                RANSAC_REPAIR_ITERATIONS > 0
                and len(symbols) <= RANSAC_REPAIR_MAX_SYMBOLS
                and len(equations) >= len(symbols)
            ):
                masks = []
                seed = 0x9E3779B97F4A7C15 ^ len(equations) ^ (len(symbols) << 16)
                for subset, y in equations:
                    mask = 0
                    for idx in subset:
                        mask |= 1 << idx
                    masks.append(mask)
                    seed ^= (mask * 0xBF58476D1CE4E5B9) & 0xFFFFFFFFFFFFFFFF
                    seed ^= int(y) * 0x94D049BB133111EB
                ...
                for _ in range(RANSAC_REPAIR_ITERATIONS):
                    order = rng.permutation(len(equations)).astype(np.int32)
                    solved = solve_equation_basis(order, masks)
                    if solved is None:
                        continue
                    conflicts = equation_conflicts(solved)
            ...
            incidences: List[List[int]] = [[] for _ in symbols]
            residuals: List[int] = []
            for eq_idx, (subset, y) in enumerate(equations):
                lhs = 0
                for idx in subset:
                    assert symbols[idx] is not None
                    lhs ^= int(symbols[idx])
                    incidences[idx].append(eq_idx)
                residuals.append((lhs ^ int(y)) & symbol_mask)

            conflicts = sum(1 for residual in residuals if residual != 0)
            if conflicts == 0:
                return

            for _pass in range(REPAIR_PASSES):
                changed = False
                for idx, current in enumerate(symbols):
                    ...
                    for candidate in range(symbol_q):
                        if candidate == current:
                            continue
                        delta = int(current) ^ int(candidate)
                        candidate_conflicts = conflicts
                        for eq_idx in incidences[idx]:
                            before = residuals[eq_idx] != 0
                            after = (residuals[eq_idx] ^ delta) != 0
                    ...
                    if best_delta:
                        for eq_idx in incidences[idx]:
                            residuals[eq_idx] ^= best_delta
                        conflicts = best_conflicts

                    symbols[idx] = best_value
                    changed = changed or (best_value != current)
        ...
        def repaired_message() -> np.ndarray | None:
            if any(v is None for v in symbols_a) or any(v is None for v in symbols_b):
                return None

            if EQUATION_BANK_REPAIR:
                repair_symbol_vector(symbols_a, equations_a)
                repair_symbol_vector(symbols_b, equations_b)

            return maybe_done()

        def remember_equation(
            x: int,
            y: int,
            k: int,
            cdf: List[float],
            ...
            if not EQUATION_BANK_REPAIR or k <= 0:
                return
            subset = subset_from_x(x, k, cdf, salt, singleton_limit=k)
            equations.append((subset, int(y) & symbol_mask))

        def add_equation_to_peeler(
            x: int,
            y: int,
            k: int,
            cdf: List[float],
            ...
            singleton_votes: List[Dict[int, int]],
        ) -> None:
            if k <= 0:
                return

            if SINGLETON_CONFIRMATIONS <= 1:
                peel_add_equation(x, y, k, cdf, salt, symbols, pending)
            ...
            if len(unknown) == 1:
                ...
                votes = singleton_votes[idx]
                rhs &= symbol_mask
                votes[rhs] = votes.get(rhs, 0) + 1
                if votes[rhs] >= SINGLETON_CONFIRMATIONS and symbols[idx] is None:
                    symbols[idx] = rhs
                    peel_propagate(symbols, pending)
                return

            pending.append((unknown, rhs & symbol_mask))
            peel_propagate(symbols, pending)
        ...
        def commit_pending_edge() -> None:
            ...
            if (not src_phase) and dst_phase:
                if k1 > 0:
                    remember_equation(x, y, k1, cdf_a, salt_a, equations_a)
                if k1 > 0 and x not in seen_a:
                    seen_a[x] = y
                    add_equation_to_peeler(
                        x,
                        y,
                        k1,
                        cdf_a,
                ...
                if k2 > 0:
                    remember_equation(x, y, k2, cdf_b, salt_b, equations_b)
        ...
        while True:
            ...
            if done is not None:
                if not EQUATION_BANK_REPAIR:
                    return done
                if repair_extra_left is None:
                    repair_extra_left = REPAIR_EXTRA_PACKETS
                if repair_extra_left <= 0:
            ...
            if repair_extra_left is not None:
                repair_extra_left -= 1
