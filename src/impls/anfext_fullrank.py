...
def nonlinear_producer(n, m, index, d, verbose=False):
    """
    Producer for y = A F(x) where A is n×m (full row rank),
    and F(x) is an ANF monomial feature map of length m.

    Payload index enumerates ONLY matrices A in our full-rank n×m family:
        - first n columns form a full-rank n×n prefix
        - remaining m-n columns are arbitrary
    """
    # decode index -> full-rank A
    A = unrank_fullrank_nm(n, m, index)
    assert d == count_fullrank_nm(n, m), "d must equal count_fullrank_nm(n,m)"
