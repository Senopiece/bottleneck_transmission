...
def _generate_monomial_masks(n: int, m: int) -> list[int]:
    if m < 0:
    ...
    non_const = [s for s in range(1, 1 << n)]
    non_const.sort(key=lambda s: (s.bit_count(), s))

    if m - 1 > len(non_const):
        raise ValueError("m exceeds number of available monomials")

    return non_const[: m - 1] + [0]


def _anf_feature_vector(x_bits: np.ndarray, monomial_masks: list[int]) -> np.ndarray:
    xmask = 0
    for b in x_bits:
        xmask = (xmask << 1) | int(b)

    out = np.zeros(len(monomial_masks), dtype=np.uint8)
    for idx, mask in enumerate(monomial_masks):
        if mask == 0:
            out[idx] = 1
        else:
...
def _build_feature_basis(n: int, m: int, masks: list[int]):
    basis: list[int] = []
    x_basis: list[int] = []
    ...
    for x in range(1, 1 << n):
        ...
        Fx = _anf_feature_vector(xb, masks)
...
def _nonlinear_generator(n: int, masks: list[int], A: np.ndarray):
    ...
    def nonlinear_step(x: int) -> int:
        xb = int_to_vec(x)
        F = _anf_feature_vector(xb, masks)
        yb = (A @ F) % 2
...
class _ChainSamplerState:
    def __init__(
        ...
        A = (Y @ Xinv) % 2
        self._gen = _nonlinear_generator(self.n, masks, A)
    ...
    def feed(self, data: Packet | None) -> Payload | None:
        ...
        if self.prev is not None and self.prev.any():
            ...
            if key not in self.transitions:
                ...
                Fx = _anf_feature_vector(self.prev, self.masks)
...
def create_protocol(config: Config) -> Protocol:
    ...
    masks = _generate_monomial_masks(packet_bits, m)
    x_basis, X, Xinv = _build_feature_basis(packet_bits, m, masks)
