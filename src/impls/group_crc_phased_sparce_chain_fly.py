from typing import Tuple

import numpy as np

from ._interface import Config, Estimator, Message, Protocol, Sampler
from ._utils.conversions import bool_array_to_uint16, uint16_to_bool_array
from .phased_sparce_chain_fly import create_protocol as create_base_protocol

# One guard packet per a small group of chain segments. This is deliberately not per-packet
# parity: the regular packet format stays unchanged, and the receiver validates
# a group before feeding it into the normal phased sparse-chain decoder.

HASH_SEED = 0x15
HASH_MUL = 17
GROUP_SEGMENTS = 2


def _packet_value(packet: np.ndarray, mask: int) -> int:
    return int(bool_array_to_uint16(packet) & mask)


def _same_packet(a: np.ndarray, b: np.ndarray) -> bool:
    return bool(np.array_equal(a, b))


def _guard_value(
    body: list[np.ndarray],
    next_first: np.ndarray,
    packet_bitsize: int,
) -> int:
    mask = (1 << packet_bitsize) - 1
    h = HASH_SEED & mask
    for packet in body:
        value = _packet_value(packet, mask)
        h = ((h * HASH_MUL) ^ value ^ (h >> 2)) & mask

    forbidden = {_packet_value(next_first, mask)}
    if body:
        forbidden.add(_packet_value(body[-1], mask))

    while h in forbidden:
        h = (h + 1) & mask
    return h


def _guard_packet(
    body: list[np.ndarray],
    next_first: np.ndarray,
    packet_bitsize: int,
) -> np.ndarray:
    return uint16_to_bool_array(
        np.uint16(_guard_value(body, next_first, packet_bitsize)), packet_bitsize
    )


def create_protocol(config: Config) -> Protocol:
    packet_bitsize = int(config.packet_bitsize)
    if packet_bitsize <= 1:
        raise ValueError("packet_bitsize must be >= 2")

    base_protocol = create_base_protocol(config)

    def make_sampler(message: Message) -> Sampler:
        base_sampler = base_protocol.make_sampler(message)
        buf: list[np.ndarray] = []
        group_body: list[np.ndarray] = []
        group_segments = 0
        prev: np.ndarray | None = None

        while True:
            packet = next(base_sampler)

            if prev is not None and _same_packet(prev, packet):
                completed = buf[:-1]
                if completed:
                    group_body.extend(completed)
                    group_segments += 1
                    if group_segments >= GROUP_SEGMENTS:
                        guard = _guard_packet(group_body, prev, packet_bitsize)
                        for out_packet in group_body:
                            yield out_packet
                        yield guard
                        group_body = []
                        group_segments = 0
                buf = [prev, packet]
            else:
                buf.append(packet)

            prev = packet

    def make_estimator() -> Estimator:
        base_estimator = base_protocol.make_estimator()
        progress = next(base_estimator)

        buf: list[np.ndarray] = []
        group_body: list[np.ndarray] = []
        group_segments = 0
        prev: np.ndarray | None = None

        def reset_inner() -> None:
            nonlocal progress
            try:
                progress = base_estimator.send(None)
            except StopIteration:
                raise

        def flush_completed(completed: list[np.ndarray], next_first: np.ndarray) -> None:
            nonlocal progress
            nonlocal group_body
            nonlocal group_segments
            if len(completed) < 2:
                return

            group_segments += 1
            if group_segments < GROUP_SEGMENTS:
                group_body.extend(completed)
                return

            body = completed[:-1]
            observed_guard = completed[-1]
            full_body = group_body + body
            expected_guard = _guard_value(full_body, next_first, packet_bitsize)
            mask = (1 << packet_bitsize) - 1
            group_body = []
            group_segments = 0
            if _packet_value(observed_guard, mask) != expected_guard:
                reset_inner()
                return

            for out_packet in full_body:
                progress = base_estimator.send(out_packet)

        while True:
            packet = yield progress

            if packet is None:
                buf.clear()
                group_body = []
                group_segments = 0
                prev = None
                reset_inner()
                continue

            if prev is not None and _same_packet(prev, packet):
                completed = buf[:-1]
                try:
                    flush_completed(completed, prev)
                except StopIteration as exc:
                    return exc.value
                buf = [prev, packet]
            else:
                buf.append(packet)

            prev = packet

    return Protocol(make_sampler=make_sampler, make_estimator=make_estimator)


def max_message_bitsize(packet_bitsize: int) -> int:
    # Disabled for notebook auto-discovery. Grouping segments behind one guard
    # packet looked cheaper, but in practice it delays useful equations too much
    # and can exceed max_iters on small payloads.
    return 0


def expected_packets_until_reconstructed(
    gilbert_eliott_k: Tuple[float, float, float, float],
    packet_bitsize: int,
    message_bitsize: int,
):
    return None
