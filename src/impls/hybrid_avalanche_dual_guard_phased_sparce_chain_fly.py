...
# Hybrid segment protection with stronger tiny hashes and a selective second guard:
...
# - very long segments get a second independent full guard packet.
#
...
def _guard_packet(
    ...
    return uint16_to_bool_array(
        np.uint16(_guard_value(body, next_first, packet_bitsize, seed)),
        packet_bitsize,
    )
...
def create_protocol(config: Config) -> Protocol:
            ...
            if prev is not None and _same_packet(prev, packet):
                ...
                if completed:
                    ...
                    if len(completed) >= MIN_STRONG_GUARD_PACKETS:
                        ...
                        if len(completed) >= MIN_SECOND_GUARD_PACKETS:
                            yield _guard_packet(
                                completed,
                                prev,
                                packet_bitsize,
    ...
    def make_estimator() -> Estimator:
        ...
        awaiting_guard: Tuple[list[np.ndarray], np.ndarray, bool, int, bool] | None = None
        ...
        while True:
            ...
            if awaiting_guard is not None:
                completed, next_first, inband_ok, guard_stage, first_guard_ok = awaiting_guard
                awaiting_guard = None
                observed_guard = _packet_value(packet, full_mask)
                expected_guard = _guard_value(
                    completed,
                    next_first,
                    packet_bitsize,
                    CRC2_HASH_SEED if guard_stage == 2 else CRC_HASH_SEED,
                )
                guard_ok = observed_guard == expected_guard

                if guard_stage == 1 and len(completed) >= MIN_SECOND_GUARD_PACKETS:
                    awaiting_guard = (
                        completed,
                        next_first,
                        inband_ok,
                ...
                try:
                    if inband_ok and first_guard_ok and guard_ok:
            ...
            if stream_prev is not None and bool(packet[0]) == bool(stream_prev[0]):
                ...
                if completed:
                    ...
                    if len(completed) >= MIN_STRONG_GUARD_PACKETS:
                        awaiting_guard = (completed, next_first, inband_ok, 1, True)
                        continue
