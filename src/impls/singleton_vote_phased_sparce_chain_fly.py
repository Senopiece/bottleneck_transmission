...
# - require repeated/majority singleton observations before peeling them;
# - validate/repair the final solution against all collected equations.

COMMIT_LAG = 1
MIN_PEEL_WEIGHT = 1.0
SINGLETON_MIN_PEEL_WEIGHT = 2.0
REPAIR_PASSES = 3
VALIDATION_EVENTS = 4
...
def create_protocol(config: Config) -> Protocol:
    ...
    if input_bits > MAX_PREFIX_INPUT_BITS:
        raise ValueError(
            "singleton_vote_phased_sparce_chain_fly is tailored for small packet sizes: "
    ...
    if message_bitsize > max_bits:
        raise ValueError(
            "message_bitsize too large for singleton_vote_phased_sparce_chain_fly: "
    ...
    def make_estimator() -> Estimator:
        ...
        def add_equation(
            ...
            if (not src_phase) and dst_phase:
                ...
                threshold = (
                    SINGLETON_MIN_PEEL_WEIGHT if x < k1 else MIN_PEEL_WEIGHT
                )
                ranked = sorted(
                ...
                if (
                    ...
                    and best_weight > second_weight
                    and x not in peeled_x_a
                ):
                    y_majority = ranked[0][0]
                    peeled_x_a.add(x)
                    peel_add_equation(
                ...
                threshold = (
                    SINGLETON_MIN_PEEL_WEIGHT if x < k2 else MIN_PEEL_WEIGHT
                )
                ranked = sorted(
