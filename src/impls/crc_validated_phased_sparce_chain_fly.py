...
# Same packet format as phased_sparce_chain_fly, but encode a small end-to-end
# CRC inside the message payload. This is not per-packet parity: packet_bitsize
# and symbol_bits stay unchanged. The CRC lets the receiver avoid returning an
# obviously poisoned reconstruction after only a bounded number of extra events.
#
# Same sender path as phased_sparce_chain_fly, but a more conservative receiver:
...
def crc_bits(message: Message, bit_count: int = CRC_BITS) -> np.ndarray:
    crc = CRC_INIT
    for bit in message:
        top = (crc >> 15) & 1
    ...
    out = np.empty(bit_count, dtype=np.bool_)
    for i in range(bit_count):
        out[i] = bool((crc >> (bit_count - 1 - i)) & 1)
    return out


def append_crc(message: Message) -> np.ndarray:
    return np.concatenate((message, crc_bits(message)))


def has_valid_crc(encoded_message: np.ndarray, payload_bitsize: int) -> bool:
    if encoded_message.shape[0] != payload_bitsize + CRC_BITS:
        return False
    payload = encoded_message[:payload_bitsize]
    actual = encoded_message[payload_bitsize:]
    return bool(np.array_equal(crc_bits(payload), actual))


def create_protocol(config: Config) -> Protocol:
    packet_bitsize = int(config.packet_bitsize)
    payload_bitsize = int(config.message_bitsize)
    encoded_bitsize = payload_bitsize + CRC_BITS
    ...
    if payload_bitsize > max_bits:
        raise ValueError(
            "message_bitsize too large for crc_validated_phased_sparce_chain_fly: "
            f"max {max_bits} for packet_bitsize={packet_bitsize}"
        )

    half_bits_a = (encoded_bitsize + 1) // 2
    half_bits_b = encoded_bitsize - half_bits_a
    ...
    def make_sampler(message: Message) -> Sampler:
        return base_protocol.make_sampler(append_crc(message))
    ...
    def make_estimator() -> Estimator:
        ...
        def maybe_done() -> np.ndarray | None:
            ...
            if has_valid_crc(encoded, payload_bitsize):
                return encoded[:payload_bitsize]
            ...
            # Keep packet overhead bounded. If the bounded validation window did
            # not repair the candidate, return best-effort payload so the metric
            # records the failure instead of turning this into an unbounded wait.
            return encoded[:payload_bitsize]
...
def max_message_bitsize(packet_bitsize: int) -> int:
    ...
    if input_bits > MAX_PREFIX_INPUT_BITS:
        return 0
    return max(0, 2 * z * (1 << input_bits) - CRC_BITS)
